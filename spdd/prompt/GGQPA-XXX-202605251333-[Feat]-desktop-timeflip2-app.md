...
1. Desktop Product Architecture:
   ...
   - Treat the macOS menu bar or tray status as a secondary surface that mirrors the active task and connection state and opens the full Wails window.
...
4. Tracking and History:
   ...
   - Represent a pause side as a local facet assignment whose active state pauses task-session recording without requiring a special device mode.
...
8. `MenuBarService` depends on `Store`, `TrackingService`, and `EventBus` to show current state and open the main window.
...
1. Desktop Shell Layer: Wails bootstrap, window lifecycle, menu-bar setup, frontend asset binding, and application startup/shutdown.
...
3. Application Service Layer: device orchestration, connection management, task assignment, tracking interpretation, history reconciliation, and menu-bar state.
...
7. Frontend Presentation Layer: app layout, device list, pairing/unpairing wizard, current-state dashboard, facet editor, history task-session list, settings, and menu-bar affordances.
...
4. Constraints:
   ...
   - Keep generated frontend assets and build outputs out of version control.
...
2. Attributes:
   ...
   - `FacetAssignment`: maps a device facet to a task, local label/icon metadata through the task, optional Pomodoro limit, pause-side intent, and effective timestamp.
   - `DeviceState`: stores current connection, facet, pause, lock, battery, system, and update state.
   - `DeviceEventRecord`: stores normalized raw/interpreted device events for idempotent history reconciliation.
   - `TaskSession`: stores interpreted work intervals with task assignment snapshot and source event boundaries.
3. Methods:
...
4. Constraints:
   ...
   - A pause-side assignment must not create a normal task session while active.
...
2. Core Methods:
   ...
   - `SaveDeviceState(ctx, state)`, `GetDeviceState(ctx, deviceID)`: persist latest state for dashboard and menu-bar startup display.
...
2. Core Methods:
   ...
   - `RefreshDeviceState(ctx, deviceID)`: connects/uses active session, authorizes with stored password, reads current device state, saves it, and emits frontend/menu-bar updates.
...
2. Attributes:
   - `activeSessions`: map - active device session handles keyed by device ID.
   - `failureCounts`: map - consecutive failed reconnect attempts per device.
   - `retryState`: map - current retry cadence per device.
   - `streamCancel`: map - event stream cancellation functions.
3. Methods:
   ...
   - `Stop(ctx)`: cancels streams, closes sessions, and persists final state before app shutdown.
4. Constraints:
...
2. Core Methods:
   ...
   - `ApplyDeviceEvent(ctx, event)`: records event idempotently, updates latest state, and applies session-boundary rules.
   ...
   - `PauseTracking(ctx, deviceID, source)`: closes open session and marks tracking paused.
   - `ResumeTracking(ctx, deviceID, source)`: starts a session for the current active non-pause facet if known and not locked.
   - `CloseOpenSession(ctx, deviceID, endedAt, sourceEvent)`: closes an active session with duration validation.
2. Business Logic:
   - Undefined facet closes or suspends active session rather than assigning time to an unknown task.
   - Pause-side facet closes the current task session and marks tracking paused locally.
...
2. Core Methods:
   - `ImportDeviceHistory(ctx, deviceID)`: reads from the last persisted checkpoint, stores new event records idempotently, and updates checkpoint only after successful reconciliation.
   - `ReconcileEventsToSessions(ctx, deviceID, events)`: orders events by device event number and occurrence time, then applies tracking rules to produce sessions.
   - `ListTaskSessions(ctx, filter)`: returns sessions filtered by device, date range, task, or facet.
   - `GetCurrentSession(ctx, deviceID)`: returns the active session for dashboard and menu-bar display.
3. Business Logic:
...
2. Methods:
   ...
   - `SaveTapPauseSettings(request)`: updates tap, auto-pause, lock, pause, or pause-side settings.
...
2. Views:
   ...
   - History: task-session list with task, facet, start, end, duration, source, and current/open session.
   - Settings: stored devices, reconnect policy defaults, timeout settings, and password update flow without displaying existing password values.
3. Interaction Logic:
...
### Implement Menu Bar Service - Current State Indicator
1. Responsibility: Provide a macOS control-centre/menu-bar-style status surface that reflects current tracking state and opens the main app window.
2. Methods:
   - `UpdateMenuBarState(state)`: sets status label/icon according to connected/offline/paused/current-task state.
   - `OpenMainWindow()`: brings the Wails window to foreground.
   - `PauseOrResumeCurrentDevice()`: optional quick action if supported by Wails/menu-bar integration without compromising the main UI.
3. Constraints:
   - Menu-bar state must be a mirror of persisted/app service state, not a separate source of truth.
   - If full menu-bar support is constrained by Wails capabilities, implement the service boundary and a minimal open-window action first.
...
4. Constraints:
   ...
   - Raw BLE trace support is not part of the first desktop app unless explicitly added later.
...
1. Package Organization:
   ...
   - Use `internal/services` for device, connection, tracking, task, history, config, and menu-bar services.
...
8. Business Rule Constraints:
   ...
   - A pause-side facet closes the current work session and marks tracking paused locally.
...
9. UI Constraints:
   ...
   - The history screen must show task sessions with start, end, duration, task, facet, and source state.
