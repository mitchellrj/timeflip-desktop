...
type advertisedNameProvider interface {
	AdvertisedName(context.Context, timeflip.DeviceID) (string, bool)
}
...
func (t *tracingTransport) AdvertisedName(ctx context.Context, id timeflip.DeviceID) (string, bool) {
	provider, ok := t.inner.(advertisedNameProvider)
	if !ok {
		return "", false
	}
	return provider.AdvertisedName(ctx, id)
}
...
func (c *tracingConnection) Read(ctx context.Context, ch timeflip.CharacteristicID) ([]byte, error) {
	...
	c.log.event("read_result", c.deviceID, ch, payload, traceReadError(ch, err))
	return payload, err
}
...
func traceReadError(ch timeflip.CharacteristicID, err error) error {
	...
	if normalizeTraceCharacteristic(ch) == "0X2A00" && errors.Is(err, timeflip.ErrProtocol) {
		return nil
	}
	return err
}

func normalizeTraceCharacteristic(ch timeflip.CharacteristicID) string {
